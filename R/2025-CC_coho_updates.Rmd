...
* Updated spawner estimates, where available, through 2024 (file [here](https://github.com/Pacific-salmon-assess/CC_coho/blob/main/Data/Coho_Brood_MASTER_2025_update.xlsx)). Previous estimates were through 2020.
...
* Refit the AR-1 time varying model from Wilson et al. 2025 (JAGS model [here](https://github.com/Pacific-salmon-assess/CC_coho/blob/main/JAGS/forecasting%20model%20ar1.jags)) to the updated data described above.
...
These analyzes have not yet attempted to incorporate the StAD SIL (steam inspection log) review of data quality in part because it has so far been limited to only a portion of the coho populations considered. With a more complete review of SILs for the systems considered in this analysis we can adapt the model to account for confidence in estimates (e.g., in likelihood by a assuming a large variance with the spawner observations that were are much less confident in). 
...
pop_group <- spawn_data |>
  filter(!is.na(total_run4),
         !is.na(escapement),
         group==4) |>
  mutate(unknown = as.numeric(escapement),
         high = as.numeric(esc_2),
         returns = as.numeric(total_run4))|>
  pivot_longer(cols=c("unknown","high"), names_to = "survey_quality", values_to = "spawners") |>
...
For systems that have had a detailed review of SILs (steam inspection logs) those observations that are considered high quality are labelled " review of data quality "high" and colored red. Long-term average spawner abundance is denoted with a dashed grey line. 
