...
* Assumed all fish mature as 3 or 4 year olds by lumping the very small (<0.05%) assumed contribution of 5 year olds together with 4 year olds. This allows for a greater number of “complete” spawner-recruitment observations 

* Adapted the time varying productivity model from Wilson et al. 2025 (JAGS model [here](https://github.com/Pacific-salmon-assess/CC_coho/blob/main/JAGS/model_fitting%20tr1%20extra%20variance.jags)) to have separate variances for spawner observations that were deemed to be of "high" and "low" or unknown quality, and then fit to the updated data described above.
...
Data quality for these analyses was based on a detailed review of SILs (steam inspection logs) by NC StAD. We modified the existing model to allow the variances in the likelihood for model fit to spawner observations to vary as a function of data quality. Specifically we assumed a larger variance on observations of low (or unknown) quality than those that have been identified as high quality via an informative half-Cauchy prior (scale = 1) on high quality observations and a weak prior (scale = 5) on low (or unknown) quality observations (figure below). 

Code to reproduce analysis, and this doc, can be found [here](https://github.com/Pacific-salmon-assess/NCC_coho).

```{r plot, echo=FALSE, fig.align = "center", out.width = '55%', fig.cap="Illustration of prior distributions used for high and low (unknown) quality spawner abundance estimates, where high quanlity observations are assumed to arise from a half-Cauchy distribution with scale =1, and low (unknown) quality observations are assumed to arise from a distribution with scale =5. "}
# Define half-Cauchy PDF function
dhalfcauchy <- function(x, scale = 1) {
  ifelse(x >= 0, (2 / pi) * (scale / (x^2 + scale^2)), 0)
}
...
data <- data.frame(
  x = rep(x, times = length(scales)),
  density = unlist(lapply(scales, function(s) dhalfcauchy(x, scale = s))),
  scale = factor(rep(scales, each = length(x)))
)
...
pop_coefs <- as.data.frame(ranef(model)) |>
  ...
  add_row(population = "region", '% change (3-gen)' = round(as.numeric((exp(model_s$coefficients[2,1]*(3*4))-1)*100))) |>
  select(population,'% change (3-gen)')
