...
dq <- matrix(nrow=n.years,ncol=n.pops,NA)
...
sigma_sd <- matrix(c(apply(logRS4,2,sd,na.rm=TRUE),apply(logRS4,2,sd,na.rm=TRUE)),nrow=2,ncol=58)
spawn_sd <- matrix(c(apply(log(spawn),2,sd,na.rm=TRUE),apply(log(spawn),2,sd,na.rm=TRUE)),nrow=2,ncol=58)
...
ggsave("Figures/CO_alpha_trend_pops_NCC_2024_updateDQ.jpeg", width = 9, height=6,units="in", dpi=600)
